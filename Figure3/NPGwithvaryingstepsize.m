...
eta_BS_2=[0.1]; %step size \eta for noise 10^-2
eta_BS_0=[]; %step size for noise 10^-0
eta_BS=[eta_BS_4,eta_BS_2,eta_BS_0];
episode=10;
varying=zeros(1,episode); %adaptive step size until i=15
for i=1:15
    varying(1,i)=1.05^i;
end
for i=16:episode
...
for counter=1:num_BS
    ...
    for i=1:episode
        ...
        kcurrentNBS=kcurrentNBS-varying(counter)*eta_BS(counter)*nablaK*inv(SigmaK);
        catch
            break;
        end
    end
...
for i=1:episode
    BS4(i,1)=(ckBS(i,1)/100-cKstar)/(cKstar); % ck for noise level 10^(-2)
end
figure;
