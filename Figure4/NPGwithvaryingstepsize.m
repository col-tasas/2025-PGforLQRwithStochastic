%% Parameters 
% % here is only the number of iterations, the adaptive step size is
% defined at line 51
eta_BS_4=[]; 
eta_BS_2=[1,1,1,1,1]; 
eta_BS_0=[]; 
...
for counter=1:num_BS
    ...
    for i=1:episode
        ...
        kcurrentNBS=kcurrentNBS-0.09/(1+2*ckBS(i,counter))*nablaK*inv(SigmaK); %adaptive step size is here
        catch
            break;
        end
    end
...
for i=1:episode
    BS4(i,2)=(ckBS(i,1)/100-cKstar)/(cKstar); % ck for noise level 10^(-2)
end
figure;
loglog(BS4,'LineStyle',':','Color','red','LineWidth',1.5)
hold on
ylim([0.01,2])
xlim([0,episode])
grid on
...
legend('$\mathrm{NPG}~\Sigma_w=10^{-2}I~ \eta_i=1.01^i\eta$','interpreter','latex','FontSize',9)
save("Varyingpartnew1.mat")
%% Required functions
